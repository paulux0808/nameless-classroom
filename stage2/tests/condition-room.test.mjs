...
test('escape objects can be reached separately and leave the central exit clear',()=>{
 ...
 assert.equal(target([-1.5,1.62,-.1],[-1.8,.69,-1.33]),'escape:cabinet');
...
test('the initial camera frames the first envelope and both work objects without navigation labels',()=>{
 ...
 for(const point of [[.12,.79,.27],[-1.8,.8,-1.33],[1.65,1,-1.01]]){const p=new T.Vector3(...point).project(camera);assert.ok(Math.abs(p.x)<.9&&Math.abs(p.y)<.9&&p.z<1,'object outside opening view');}
