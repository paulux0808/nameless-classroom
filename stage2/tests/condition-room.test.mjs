...
test('escape objects can be reached separately and leave the exit clear',()=>{
 ...
 const [cx,,cz]=ch.roomObjects.cabinet,[bx,,bz]=ch.roomObjects.bench;
 assert.equal(target([cx,1.62,cz+1.8],[cx,.69,cz+.32]),'escape:cabinet');
 ...
 assert.equal(target([bx,1.62,bz+1.8],[bx,1.1,bz+.34]),'escape:bench');
 assert.equal(target([ch.door.pos[0],1.62,ch.door.pos[2]+2],[ch.door.pos[0],1.62,ch.door.pos[2]]),undefined);
...
test('the initial camera frames the first envelope and both work objects without navigation labels',()=>{
 ...
 const [cx,,cz]=ch.roomObjects.cabinet,[bx,,bz]=ch.roomObjects.bench;
 for(const point of [ch.anchors.reportSlot,[cx,.8,cz+.32],[bx,1.1,bz+.34]]){const p=new T.Vector3(...point).project(camera);assert.ok(Math.abs(p.x)<.9&&Math.abs(p.y)<.9&&p.z<1,'object outside opening view');}
