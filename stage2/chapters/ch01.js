...
(function (root, factory) {
  ...
  return {
    ...
    /* ── 세트 ──────────────────────────────────────────────────────────
       한 세트는 "같은 얘기를 하는 문장 셋"(장마다 하나씩)이다.
       agree=true 면 셋이 같은 얘기를 한다 — 함정이다. 말만 다르게 썼을 뿐
       뜻이 같으니, 이걸 어긋났다고 고르면 틀린 것이다.
       agree=false 면 두 장은 같은 얘기를 하는데 한 장만 딴소리를 한다.
       odd 가 그 딴소리하는 장이다.

       ※ 말을 섞지 말 것: 화면과 대사 어디서도 "같은 얘기" 로만 쓴다.
          "같은 값" 이니 "같은 수치" 니 하고 섞으면, 숫자가 아닌 자리에서
          무엇을 견줘야 하는지 알 수 없게 된다. */
    sets: [
      /* ── 같은 얘기(함정) 여섯 ─────────────────────────────────────── */
      { id: "machine", label: "쓴 기계", agree: true,
        same: "셋 다 3호기 얘기입니다." },
      { id: "input", label: "시작한 값", agree: true,
        same: "셋 다 0 에서 시작했다는 얘기입니다." },
      { id: "steps", label: "나눈 횟수", agree: true,
        same: "셋 다 세 번에 나눠 했다는 얘기입니다." },
      { id: "result", label: "나온 답", agree: true,
        same: "셋 다 2.9107 입니다." },
      /* 단위가 달라 보이지만 같은 시간이다 */
      { id: "dur", label: "붙잡고 있던 시간", agree: true,
        same: "한 시간 사십오 분이나 105분이나 같은 시간이고, 두 시간이 안 " +
              "걸렸다는 것도 같은 얘기입니다." },
      /* 자릿수를 세는 방식만 다르다 */
      { id: "digits", label: "소수점 자리", agree: true,
        same: "네 자리까지 적는 것과 0.0001 자리까지 적는 것, 다섯째를 버리는 것은 " +
              "다 같은 얘기입니다." },

      /* ── 딴소리 여섯 ─────────────────────────────────────────────── */
      { id: "zero", label: "눈금 맞추기", agree: false, odd: "r1",
        wrong: "1차만 어제 맞춰 둔 걸 그대로 썼다고 합니다.",
        ok:    "세 번 다 그날 아침에 새로 맞췄다고 합니다." },
      { id: "std", label: "본 표", agree: false, odd: "r2",
        wrong: "2차만 B 표를 봤다고 합니다.",
        ok:    "세 번 다 A 표를 봤다고 합니다." },
      { id: "deck", label: "카드 뭉치", agree: false, odd: "r3",
        wrong: "3차만 117번 뭉치를 썼다고 합니다.",
        ok:    "세 번 다 114번 뭉치를 썼다고 합니다." },
      { id: "redo", label: "전기 나간 뒤", agree: false, odd: "r3",
        wrong: "3차만 끊긴 데부터 이어서 했다고 합니다.",
        ok:    "세 번 다 처음부터 다시 했다고 합니다." },
      { id: "witness", label: "옆에 있던 사람", agree: false, odd: "r2",
        wrong: "2차만 혼자 하고 혼자 읽었다고 합니다.",
        ok:    "세 번 다 둘이서 봤다고 합니다." },
      /* 두 문장을 붙여 봐야 나머지 하나가 안 맞는다는 게 드러난다.
         1.2 에서 2.6 이면 커진 것이므로 1차와 2차는 같은 얘기고,
         줄었다고 적은 3차만 성립할 수 없다. 셋 중 둘만 맞을 수 있다. */
      { id: "ratio", label: "커졌나 줄었나", agree: false, odd: "r3",
        wrong: "1.2 에서 2.6 이면 커진 겁니다. 3차만 거꾸로 적었습니다.",
        ok:    "세 번 다 두 배쯤 커졌다고 합니다." }
    ],
    ...
    /* ── 수정본 ──────────────────────────────────────────────────────────
       반려한 여섯 자리만 다시 해서 적었다. 이제 셋이 같은 얘기를 한다. */
    revisedReports: [
      {
      ...
      },
      {
        ...
        body: [
          ...
          [ { id: "rr3_ratio", set: "ratio", t: "첫 번째에서 두 번째로 가면서 값이 두 배 남짓 커졌다." },
            ...
            "별일 없었음. RICHARD. 덧붙임 — 올리기 전에 세 장이 같은 얘기를 하는지 먼저 봤다." ]
        ]
      }
    ],
    ...
    /* ── 대사 ───────────────────────────────────────────────────────────
       편지 화면에는 안내문이 없다. 무엇을 해야 하는지도, 얼마나 남았는지도
       여기서만 알 수 있다. 그러니 처음 한 번은 제대로 설명해야 한다.

       ※ 말을 섞지 말 것: "같은 얘기" 로만 쓴다. 같은 값이니 같은 수치니
          하고 섞으면, 숫자가 없는 자리에서 무엇을 견주라는 건지 알 수 없다. */
    lines: {
      /* 문 밖에서. 말이 아니라 소리다 — 한 줄씩 늘어난다. */
      knock: ["똑.", "똑. 똑.", "똑. 똑. 똑."],

      /* 걸어 들어와서 — 처음 한 번만 이렇게 길게 */
      submission: [
        "박사님. 계산부 RICHARD 입니다.",
        "적분 검산한 거 가지고 왔습니다.",
        "같은 계산을 세 번 돌렸습니다. 기계도 같고, 넣은 값도 같고, 하는 " +
        "방법도 같게요.",
        "세 번 다 2.9107 이 나왔습니다. 소수점 자리까지 똑같습니다.",
        "세 번 다 나온 값은 괜찮다고들 했고요.",
        "그래도 한번 봐 주십시오. 저희끼리 괜찮다고 하는 것보다는 나을 테니까요.",
        "한 번 돌린 걸 한 장씩 적었습니다. 세 장을 책상에 올려 뒀습니다.",
        "보시는 법은 이렇습니다. 세 장이 같은 얘기를 하고 있는 자리를 " +
        "찾으시면 됩니다.",
        "예를 들어 어느 장에는 기계 얘기가 있고, 다른 장에도 기계 얘기가 " +
        "있습니다. 그 셋이 한 자리입니다.",
        "그 셋을 골라 주십시오. 한 장에서 한 문장씩, 세 문장입니다.",
        "다만 두 장은 같은 얘기를 하는데 한 장만 딴소리를 하는 자리가 " +
        "있다면, 그런 것만 골라 주십시오.",
        "말만 다르게 쓴 건 딴소리가 아닙니다. 같은 얘기를 다르게 적었을 " +
        "뿐인 자리도 있을 겁니다.",
        "다 고르셨으면 저를 부르십시오. 제가 보겠습니다."
      ],

      /* 아직 아무것도 안 골랐을 때 */
      probing: [
        "아직 고르신 데가 없습니다.",
        "세 장이 같은 얘기를 하는 자리부터 찾아 보시죠.",
        "두 장은 같은데 한 장만 딴소리를 하는 자리, 그것만 고르시면 됩니다."
      ],

      /* 고른 게 다 맞는데 아직 남았을 때 — 몇 개인지는 말하지 않는다 */
      notYet: [
        "여기까지는 맞습니다.",
        "아직 남았습니다. 더 보십시오."
      ],

      /* 같은 얘기를 하는 자리를 딴소리라고 골랐을 때.
         뒤에 그 세트의 same 이 붙는다. */
      pushback: [
        "이건 아닙니다, 박사님.",
        "그 세 문장은 같은 얘기를 하고 있습니다."
      ],
      pushbackTail: [
        "말만 다르게 썼지 같은 소립니다. 다시 봐 주십시오."
      ],

      /* 딴소리하는 자리를 전부, 그것만 골랐을 때 */
      conceded: [
        "……잠깐만요.",
        "본 표도 다르고, 눈금 맞춘 것도 다르고, 카드 뭉치도 다릅니다.",
        "전기 나간 뒤에 어떻게 했는지도, 옆에 사람이 있었는지도 회차마다 " +
        "다르게 적었군요.",
        "값이 커졌는지 줄었는지까지…… 1.2 에서 2.6 이면 커진 겁니다. " +
        "줄었다고 적어 놨네요.",
        "같은 조건으로 세 번 했다고 해 놓고, 세 번 다 다르게 한 겁니다.",
        "그러면 세 번 다 2.9107 이 나온 게, 그냥 그렇게 된 걸 수도 있다는 " +
        "뜻이고요.",
        "다시 돌리고 오겠습니다."
      ],
      ...
      rejected: [
        "반려. 알겠습니다.",
        "고르신 자리 맞춰서 처음부터 다시 하겠습니다."
      ],

      /* 나갔다가 돌아와서 — 처음 한 번 */
      revised: [
        "다시 했습니다, 박사님.",
        "고르셨던 자리만 다시 적어서 올렸습니다.",
        "같은 자리를 다시 골라 보시면, 이번엔 세 장이 같은 얘기를 합니다."
      ],
      /* 그 뒤로는 짧게 */
      revisedAgain: [
        "아직 남았습니까? 마저 봐 주십시오.",
      ...
      approved: [
        "죄송합니다, 박사님.",
        "답만 맞으면 되는 줄 알았습니다.",
        "세 번 했다는 말이랑, 세 번 똑같이 했다는 말이 다른 말이라는 걸 " +
        "몰랐습니다.",
        "괜찮다고들 한 것도 사실은 세 번 다 다른 걸 보고 한 소리였고요.",
        "다음부터는 올리기 전에 세 장이 같은 얘기를 하는지 먼저 보겠습니다.",
        "가 보겠습니다. 문은 열어 두겠습니다."
      ],
